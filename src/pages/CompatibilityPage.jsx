...
const CompatibilityPage = () => {
  ...
  const getCompatibilityStatus = (destinyNum, partnerDestinyNum) => {
    ...
    if (compatData.good && compatData.good.includes(partnerDestinyNum))
      return "good";
    if (compatData.not && compatData.not.includes(partnerDestinyNum))
      return "not";
    if (compatData.avoid && compatData.avoid.includes(partnerDestinyNum))
      return "avoid";
    if (compatData.neutral && compatData.neutral.includes(partnerDestinyNum))
      return "neutral";
  ...
  const handleAnalyzeCompatibility = (e) => {
    ...
    if (!destiny1 || !destiny2) {
      setError(
        "One or both dates of birth are invalid. Please use DDMMYYYY format.",
      );
      return;
    }
    ...
    setCompatibilityReport({
      ...
      overallStatus:
        status1to2 === "good" && status2to1 === "good"
          ? "good"
          : status1to2 === "avoid" || status2to1 === "avoid"
            ? "avoid"
            : status1to2 === "not" || status2to1 === "not"
              ? "not"
              : "neutral",
    });
  };
        ...
        <div className="max-w-5xl mx-auto relative z-10">
          ...
          >
            ...
            <p className="text-yellow-200/70 mt-2">
              Measure synergy for partners, teams and relationships using
            ...
            >
              ...
              <form onSubmit={handleAnalyzeCompatibility} className="space-y-6">
                ...
                <div className="p-3 sm:p-4 bg-gradient-to-r from-yellow-500/10 to-amber-500/10 border border-yellow-400/30 rounded-lg">
                  <p className="text-xs sm:text-sm text-yellow-300 mb-4 font-semibold">
                    Select Two Different Family Members to Check Compatibility:
                  </p>
                ...
                {!selectedPerson1Id || !selectedPerson2Id ? (
                  <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-lg text-center text-amber-300 text-sm">
                    Please select two different family members from above to
          ...
          {compatibilityReport && (
            ...
            >
              ...
              <div className="pdf-page-break-after space-y-8">
                ...
                <div
                  className={`p-8 rounded-xl bg-gradient-to-r ${getStatusColor(compatibilityReport.overallStatus)} bg-opacity-20 border-2 border-white/20 text-center`}
                >
                  <p className="text-sm uppercase tracking-widest font-semibold text-white/80 mb-2">
                    Overall Compatibility Status
                  </p>
                  <p className="text-6xl mb-3">
                    {getStatusIcon(compatibilityReport.overallStatus)}
                  </p>
                  <p className="text-3xl font-bold text-white mb-2">
                    {getStatusLabel(compatibilityReport.overallStatus)}
                  </p>
                    ...
                    <div className="space-y-3">
                      <div className="bg-gray-900/50 p-4 rounded-md text-center">
                        ...
                        <p className="text-5xl font-bold text-yellow-300 mb-2">
                          {compatibilityReport.person1.destiny}
                        </p>
                      ...
                      <div
                        className={`p-3 rounded-md border-l-4 ${
                          compatibilityReport.person1.status === "good"
                            ? "bg-green-900/20 border-green-400"
                            : compatibilityReport.person1.status === "avoid"
                              ? "bg-red-900/20 border-red-400"
                              : compatibilityReport.person1.status === "not"
                                ? "bg-orange-900/20 border-orange-400"
                                : "bg-blue-900/20 border-blue-400"
                        }`}
                      >
                        <p className="text-xs text-gray-400 mb-1">
                          Compatibility with {compatibilityReport.person2.name}
                        </p>
                        <p className="text-lg font-bold text-yellow-300">
                          {getStatusLabel(compatibilityReport.person1.status)}
                        </p>
                      </div>
                      <div className="bg-gray-900/50 p-3 rounded-md">
                        <p className="text-xs text-gray-400 mb-2">
                          Destiny Archetype Description
                        </p>
                        <p className="text-sm text-gray-300 leading-relaxed">
                          {compatibilityReport.person1.description}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-800/60 p-4 sm:p-6 rounded-lg shadow-lg border border-yellow-400/20">
                    ...
                    <div className="space-y-3">
                      <div className="bg-gray-900/50 p-4 rounded-md text-center">
                        ...
                        <p className="text-5xl font-bold text-yellow-300 mb-2">
                          {compatibilityReport.person2.destiny}
                        </p>
                        <p className="text-lg text-yellow-200 font-semibold">
                          The{" "}
                          {getDestinyArchetype(
                      ...
                      <div
                        className={`p-3 rounded-md border-l-4 ${
                          compatibilityReport.person2.status === "good"
                            ? "bg-green-900/20 border-green-400"
                            : compatibilityReport.person2.status === "avoid"
                              ? "bg-red-900/20 border-red-400"
                              : compatibilityReport.person2.status === "not"
                                ? "bg-orange-900/20 border-orange-400"
                                : "bg-blue-900/20 border-blue-400"
                        }`}
                      >
                        <p className="text-xs text-gray-400 mb-1">
                          Compatibility with {compatibilityReport.person1.name}
                        </p>
                        <p className="text-lg font-bold text-yellow-300">
                          {getStatusLabel(compatibilityReport.person2.status)}
                        </p>
                      </div>
                      <div className="bg-gray-900/50 p-3 rounded-md">
                        <p className="text-xs text-gray-400 mb-2">
                        ...
                        <p className="text-sm text-gray-300 leading-relaxed">
                          {compatibilityReport.person2.description}
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className="pdf-page-break-after space-y-8">
                  ...
                  <div className="space-y-4 sm:space-y-6">
                    ...
                    <div className="bg-gray-900/50 p-3 sm:p-5 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4 mb-4">
                        ...
                        {DATA.destinyCompatibility[
                          ...
                          <div className="bg-orange-900/20 p-3 rounded-md border border-orange-400/30">
                            <p className="text-xs text-orange-300 mb-2 font-semibold">
                              Challenging
                            </p>
                            <p className="text-sm text-white">
                              {DATA.destinyCompatibility[
                      ...
                      <p className="text-sm text-gray-300 italic">
                        Partner's Destiny {compatibilityReport.person2.destiny}{" "}
                        falls under:{" "}
                    ...
                    <div className="bg-gray-900/50 p-3 sm:p-5 rounded-lg">
                      ...
                      <p className="text-sm text-gray-300 italic">
                        Partner's Destiny {compatibilityReport.person1.destiny}{" "}
                        falls under:{" "}
                        <span className="font-bold text-yellow-300">
                          {getStatusLabel(compatibilityReport.person2.status)}
                        </span>
                ...
                <div className="bg-gray-800/60 p-6 rounded-lg shadow-lg border border-yellow-400/20">
                  ...
                  <div className="prose prose-invert text-gray-300 max-w-none">
                    <p className="mb-4">
                      Numerology compatibility reveals the natural energy
                      dynamics between two people based on their Destiny
                      Numbers. This ancient Vedic wisdom can guide you toward
                      understanding, but remember:
                    </p>
                    <ul className="list-disc list-inside space-y-2">
                      <li>
                        ...
                        show natural harmony and ease in the relationship
                      </li>
                      <li>
                        <strong className="text-yellow-300">
                          Neutral Connections
                        </strong>{" "}
                        can work with mutual respect and understanding
                      </li>
                      <li>
                        <strong className="text-yellow-300">
                          Challenging Matches
                        </strong>{" "}
                        require extra effort but can lead to growth
                      </li>
                      <li>
                        <strong className="text-yellow-300">
                          Difficult Matches
                        </strong>{" "}
                        need conscious work and compromise
                      </li>
                    </ul>
                    <p className="mt-4 text-sm text-yellow-200/80">
                      Every relationship is unique. These insights are meant to
                      guide, not dictate. With awareness, communication, and
                      genuine care, any two people can create a meaningful
                      connection.
                    </p>
                  </div>
                </div>
              </div>{" "}
              {/* Close PAGE 3 wrapper */}
              <button
                onClick={() => {
                  setCompatibilityReport(null);
                  setPerson1Name("");
