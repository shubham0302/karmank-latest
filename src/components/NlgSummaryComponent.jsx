    ...
    const generateSummary = async () => {
      ...
      try {
        ...
        if (result.success && result.text) {
          ...
          setSummary(
            "Could not generate a detailed summary at this time. Please review the data below.",
          );
        ...
        let summaryMessage =
          "Could not generate a summary. Please check your backend server is running.";

        if (err.name === "AbortError") {
          errorMessage = "Request timeout: AI generation took too long (>30s).";
          summaryMessage =
            "The AI service is taking longer than expected. This might be due to Lambda cold start or Gemini API latency. Please try again.";
        } else if (
          ...
          summaryMessage =
            "Backend server is not reachable. Check if the server is running at: " +
            (import.meta.env.VITE_BACKEND_URL || "http://localhost:8080");
        } else if (err.message.includes("API key")) {
          errorMessage = "Backend API key not configured.";
          summaryMessage =
            "The backend server needs a valid Gemini API key. Please configure GEMINI_API_KEY environment variable.";
        } else if (err.message.includes("Gemini")) {
          errorMessage = "AI service error: " + err.message;
          summaryMessage =
            "Google Gemini API returned an error. Check Lambda logs for details.";
        } else {
      ...
      {!isLoading && !summary && !error && (
        <div className="text-center">
          <p className="text-indigo-200 mb-3">
            Click below to generate AI-powered insights for this section
          </p>
          <button
