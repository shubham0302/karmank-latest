...
const NlgChildBirthForecast = ({ analysis, shouldGenerate = false }) => {
  ...
  useEffect(() => {
    ...
    const generateText = async () => {
      ...
      let prompt =
        "You are a numerology expert providing a childbirth forecast. Based on the following analysis, write a concise, encouraging, and easy-to-understand forecast in 2-3 sentences.\n\nAnalysis Data:\n";
      ...
      if (!analysis.dasha3.active && !analysis.even8.active) {
        prompt +=
          "- No specific positive indicators for childbirth are active in the dashas.\n";
      }
      ...
      try {
        ...
        console.log(
          "[Childbirth Forecast] 🔒 Calling secure backend API for NLG generation...",
        );
        ...
        if (!response.ok) {
          const errorMsg =
            result.message || `API call failed with status: ${response.status}`;
        ...
        if (result.success && result.text) {
          ...
          if (result.cached) {
            console.log(
              "✅ Childbirth forecast loaded from cache (no API cost)",
            );
          } else {
        ...
        setError(
          "Could not generate the forecast at this time. Please ensure backend server is running.",
        );
        if (analysis.status === "Green") {
          setForecastText(
            "The current Dasha period shows positive indicators, suggesting a high possibility for childbirth this year.",
          );
        } else if (analysis.status === "Yellow") {
          setForecastText(
            "While some indicators are present, the overall energy for childbirth is neutral. Other factors in the chart are not strongly amplifying the possibility.",
          );
        } else {
          setForecastText(
            "No specific numerological indicators for childbirth are active in the current major Dasha periods.",
          );
        }
      } finally {
        setIsLoading(false);
  ...
  if (isLoading) {
    return (
      <div className="p-4 text-center text-indigo-200 animate-pulse">
        Generating your personalized forecast...
      </div>
    );
  }
  if (error) {
    ...
    return (
      <div className="p-4 text-center">
        <p className="text-indigo-200 mb-3">
          Click below to generate your AI-powered childbirth forecast
        </p>
        <button
