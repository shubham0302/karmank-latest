...
const ChildBirthForecastTab = ({ report, dashaReport, gender, targetDate }) => {
  ...
  if (!report || !dashaReport || !targetDate) {
    return (
      <div className="p-4 bg-red-900/50 text-red-300 rounded-lg">
        Required data (report, dashaReport, targetDate) is missing. Cannot
        render component.
      </div>
    );
  }
  ...
  const possibilityAnalysis = useMemo(() => {
    const yearlyDashaRec = dashaReport.yearlyDashaTimeline.find(
      (d) => targetDate >= d.startDate && targetDate <= d.endDate,
    );
    const mahaDashaRec = dashaReport.mahaDashaTimeline.find(
      (d) => targetDate >= d.startDate && targetDate <= d.endDate,
    );
    ...
    if (yearlyDasha === 3) {
      ...
      findings.dasha3.favorable =
        count3 === 0 || count3 === 1 || (count3 === 2 && destinyNumber === 3);
    }
    ...
    if (
      (findings.dasha3.active && findings.dasha3.favorable) ||
      findings.even8.active
    ) {
  ...
  const avoidanceAnalysis = useMemo(() => {
    if (gender !== "Female") {
      return {
        text: "This analysis is applicable only for the female chart.",
        status: "Neutral",
      };
    }
    ...
    if (yearlyDasha === undefined)
      return {
        text: "Dasha data not available for this period.",
        status: "Neutral",
      };
  ...
  return (
    <div className="space-y-8 font-sans text-white">
      ...
      >
        ...
        <p className="text-sm text-yellow-200/70 mb-4 print:text-xs print:mb-2">
          Analyzes the current forecast year to identify periods with a high
          probability of childbirth based on your Annual Dasha.
        </p>
        ...
        <div className="mt-6 pt-6 border-t border-gray-700 print:mt-4 print:pt-4">
          ...
          <p className="text-sm text-white/70 mb-4 print:text-xs print:mb-2">
            This analysis, most relevant for the female chart, determines if the
            current period is favorable for planning a pregnancy based on active
            numerological energies.
          </p>
          <StatusBlock status={avoidanceAnalysis.status}>
            <p className="font-bold text-xl text-center print:text-lg">
              {avoidanceAnalysis.text}
            </p>
          </StatusBlock>
        </div>
      </Card>
      ...
      <Card
        className="pdf-page-break-after print:hidden"
        style={{ pageBreakAfter: "always", pageBreakInside: "avoid" }}
      >
        <SectionTitle>Planned Delivery Date Validator</SectionTitle>
        <p className="text-sm text-white/70 mb-4 print:text-xs print:mb-2">
          Select a potential date for a planned delivery to receive
          numerological feedback.
        </p>
        <div className="mb-4">
          <label
        ...
        {selectedDeliveryDate && (
          <div className="space-y-4 p-4 bg-gray-900/50 rounded-lg print:space-y-2 print:p-2">
            <div className="flex justify-between items-center print:text-sm">
              ...
              <span className="font-bold text-2xl text-indigo-400 print:text-lg">
                {deliveryDateAnalysis.basicNumber}
              </span>
            </div>
            <div className="flex justify-between items-center print:text-sm">
              <span className="font-medium">Calculated Destiny Number:</span>
              <span className="font-bold text-2xl text-indigo-400 print:text-lg">
                {deliveryDateAnalysis.destinyNumber}
              </span>
            </div>
            <div
              className={`p-4 rounded-md text-center print:p-2 ${getFeedbackColor(deliveryDateAnalysis.feedback)}`}
            >
              <p className="font-bold text-xl print:text-base">
                {deliveryDateAnalysis.feedback}
              </p>
            </div>
          </div>
        )}
        <div className="mt-6 p-4 rounded-md bg-yellow-900/50 text-yellow-300 border-l-4 border-yellow-500 print:mt-3 print:p-2 print:text-xs">
          ...
          <p className="text-sm print:text-xs">
            This tool provides preliminary guidance. A complete and careful
            analysis of the date by a professional is recommended for a final
            decision.
          </p>
        </div>
      </Card>
