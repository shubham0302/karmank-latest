...
const MarriageForecastTab = ({ report, dashaReport, targetDate }) => {
    if (!report || !dashaReport) {
        console.error('[MarriageForecast] Missing required data:', { report: !!report, dashaReport: !!dashaReport });
        return null;
    }

    const marriageAnalysis = useMemo(() => {
        console.log('[MarriageForecast] Calculating analysis for targetDate:', targetDate);
        ...
        if (!yearlyDasha || !mahaDasha) {
            console.warn('[MarriageForecast] Dasha not found for targetDate:', targetDate);
    ...
    const marriageProbabilityChart = useMemo(() => {
        ...
        console.log('[MarriageForecast] Calculated probability chart:', {
            ...
            highProbYears: probabilities.filter(p => p.probability === 'High').length
        });
        ...
        <div className="space-y-6">
            ...
            <Card className="pdf-page-break-after" style={{ pageBreakAfter: 'always', pageBreakInside: 'avoid' }}>
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-6">
                    <div>
                        <h4 className="font-semibold text-lg text-yellow-300 mb-2 print:text-base print:mb-1">Major Life Period (Maha-Dasha) Influences</h4>
                        {marriageAnalysis.mahaDashaInsights.length > 0 ? marriageAnalysis.mahaDashaInsights.map((item, i) => (
                            <p key={i} className={`${item.status === 'Green' ? 'text-green-400' : 'text-red-400'} print:text-sm print:mb-1`}>{item.text}</p>
                        )) : <p className="print:text-sm">Neutral influence from current Maha Dasha.</p>}
                    </div>
                    <div>
                        <h4 className="font-semibold text-lg text-yellow-300 mb-2 print:text-base print:mb-1">Yearly (Annual Dasha) Influences</h4>
                        {marriageAnalysis.annualDashaInsights.length > 0 ?
                            marriageAnalysis.annualDashaInsights.map((item, i) => (
                                <p key={i} className={`${item.status === 'Green' ? 'text-green-400' : item.status === 'Red' ? 'text-red-400' : 'text-yellow-400'} print:text-sm print:mb-1`}>{item.text}</p>
                            )) : <p className="print:text-sm">Neutral influence from current Annual Dasha.</p>}
                    </div>
                </div>

                {/* Personality Compatibility on Same Page */}
                <div className="mt-6 pt-6 border-t border-gray-700 print:mt-4 print:pt-4">
                    <h3 className="text-xl font-bold text-yellow-300 mb-3 print:text-lg print:mb-2">Personality Compatibility Snapshot</h3>
                    <p className="mb-4 text-white/90 print:text-sm print:mb-2">As a Destiny Number <span className="font-bold text-yellow-400">{report.destinyNumber}</span>, your core life path energy influences who you connect with most naturally. {compatibilityDescription}</p>
                    <div className="grid md:grid-cols-3 gap-4 text-center print:gap-2">
                        <div className="bg-green-500/10 p-3 rounded-lg print:p-2">
                            <h4 className="font-bold text-green-400 mb-2 print:text-sm print:mb-1">Good Compatibility</h4>
                            <p className="text-2xl font-bold text-white print:text-lg">{compatibility?.good?.join(', ') || 'N/A'}</p>
                        </div>
                        <div className="bg-yellow-500/10 p-3 rounded-lg print:p-2">
                            <h4 className="font-bold text-yellow-400 mb-2 print:text-sm print:mb-1">Neutral</h4>
                            <p className="text-2xl font-bold text-white print:text-lg">{compatibility?.neutral?.join(', ') || 'N/A'}</p>
                        </div>
                        <div className="bg-red-500/10 p-3 rounded-lg print:p-2">
                            <h4 className="font-bold text-red-400 mb-2 print:text-sm print:mb-1">{compatibility?.not ? 'Not Compatible' : 'Avoid'}</h4>
                            <p className="text-2xl font-bold text-white print:text-lg">{(compatibility?.not || compatibility?.avoid || []).join(', ') || 'N/A'}</p>
                        </div>
                    </div>
                </div>
            </Card>
            ...
            <Card className="pdf-page-break-after" style={{ pageBreakAfter: 'always', pageBreakInside: 'avoid' }}>
                ...
                <div className="space-y-0.5 print:space-y-0 print:leading-tight">
                    {marriageProbabilityChart.map(item => (
                        <div key={item.age} className="flex items-center justify-between p-1 bg-gray-900/50 rounded-md print:p-0.5 print:py-0 print:bg-transparent print:border-b print:border-gray-700/30">
                            <div className="font-bold print:text-[10px] print:font-normal">{item.year} (Age {item.age})</div>
                            <div className={`px-3 py-1 text-sm font-semibold rounded-full print:px-1.5 print:py-0 print:text-[10px] print:rounded ${
