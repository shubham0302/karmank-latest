...
const RudrakshaTab = ({ report, language = "en" }) => {
    ...
    return (
      <Card>
        <p className="text-yellow-400">
          Report data not fully loaded. Please regenerate your report.
        </p>
      </Card>
    );
  }
    ...
    return (
      <Card>
        <p className="text-red-400">
          Basic number or destiny number is missing.
        </p>
      </Card>
    );
  }
    ...
    return (
      <Card className="print:mb-4" style={{ pageBreakInside: "avoid" }}>
        <h2 className="text-2xl font-bold text-yellow-400 mb-4">
          Rudraksha for Number {number} (
          {getText(rudrakshaData.planet, language)})
        </h2>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-xs sm:text-sm">
              <div className="bg-gray-900/50 p-3 rounded-md">
                <strong>Primary Mukhi:</strong>{" "}
                {getText(advancedData.primary, language)}
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md">
                <strong>Secondary Mukhi:</strong>{" "}
                {getText(advancedData.secondary, language)}
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md">
                <strong>Deity:</strong> {getText(rudrakshaData.deity, language)}
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md">
                <strong>Wearing Day:</strong>{" "}
                {getText(rudrakshaData.wearingDay, language)}
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md">
                <strong>Finger:</strong>{" "}
                {getText(rudrakshaData.finger, language)}
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md">
                <strong>Metal:</strong> {getText(rudrakshaData.metal, language)}
              </div>
              <div className="bg-gray-900/50 p-3 rounded-md col-span-2">
                <strong>Reasoning:</strong>{" "}
                {getText(advancedData.reasoning, language)}
              </div>
            </div>
            <div className="mt-4 bg-gray-900/50 p-3 rounded-md">
              ...
              <p className="text-cyan-300 italic">
                {getText(rudrakshaData.mantra, language)}
              </p>
            </div>
          </div>
          <div>
            ...
            <div className="mt-4 bg-gray-900/50 p-3 rounded-md">
              ...
              <p className="text-sm">
                {getText(rudrakshaData.method, language)}
              </p>
            </div>
          </div>
        </div>
  ...
  try {
    return (
      ...
      >
        {uniqueNumbers.map((num) => (
          <RudrakshaCard key={num} number={num} />
        ))}
      </div>
    );
  } catch (error) {
    ...
    return (
      <Card>
        <p className="text-red-400">
          An error occurred while loading Rudraksha data: {error.message}
        </p>
      </Card>
    );
  }
