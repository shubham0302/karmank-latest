...
const WelcomeTab = ({ report, userData, language = "en" }) => {
  ...
  const snapshotDescription = combination
    ? getText(combination, language)
    : "No specific insight available for this combination.";
  ...
  const nlgPrompt = useMemo(() => {
    ...
    if (report.recurringAnalysis && report.recurringAnalysis.length > 0) {
      ...
      report.recurringAnalysis.forEach((item) => {
        const influence =
          translatedAnalysis.find((a) => a.number === item.number)?.influence ||
          "";
    ...
    <div className="space-y-6">
      ...
      <div className="pdf-page-break-after space-y-6">
        ...
        <Card>
          <div className="grid lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-3">
              <h3 className="text-lg font-bold text-center text-yellow-300 mb-3">
                Base Kundli
              </h3>
                ...
                <div className="w-full max-w-full sm:max-w-xs">
                  <div className="grid grid-cols-3 gap-1.5 bg-gray-900/50 p-2 rounded-md aspect-square">
                    {[3, 1, 9, 6, 7, 5, 2, 8, 4].map((num, i) => (
                      ...
                      >
                        {report.baseKundliGrid[num] > 0
                          ? String(num).repeat(report.baseKundliGrid[num])
                          : ""}
                      </div>
                    ))}
                  </div>
              ...
              <div className="mt-2 sm:mt-3 flex flex-wrap justify-center gap-1 sm:gap-2 text-[10px] sm:text-xs">
                <div className="flex items-center gap-1.5">
                  <div
                    ...
                    style={{ backgroundColor: colorMap.basic }}
                  ></div>
                ...
                <div className="flex items-center gap-1.5">
                  <div
                    className="w-3 h-3 rounded"
                    style={{ backgroundColor: colorMap.destiny }}
                  ></div>
                ...
                <div className="flex items-center gap-1.5">
                  <div
                    className="w-3 h-3 rounded"
                    style={{ backgroundColor: colorMap.dob }}
                  ></div>
            ...
            <div className="md:col-span-1 lg:col-span-2 flex flex-col justify-center space-y-2 sm:space-y-3">
              <div className="text-center p-3 sm:p-4 bg-gradient-to-br from-rose-900/40 to-rose-800/20 border border-rose-700/50 rounded-lg">
                <p className="text-[10px] sm:text-xs text-rose-400 mb-0.5">
                  Basic Number
                </p>
                <p className="text-[10px] sm:text-xs text-rose-300/70 mb-2">
                  (Moolank)
                </p>
                <p className="text-4xl sm:text-6xl font-bold text-rose-300">
                  {report.basicNumber}
                </p>
              </div>
              <div className="text-center p-3 sm:p-4 bg-gradient-to-br from-green-900/40 to-green-800/20 border border-green-700/50 rounded-lg">
                <p className="text-[10px] sm:text-xs text-green-400 mb-0.5">
                  Destiny Number
                </p>
                <p className="text-[10px] sm:text-xs text-green-300/70 mb-2">
                  (Bhagyank)
                </p>
                <p className="text-4xl sm:text-6xl font-bold text-green-300">
        ...
        <Card className="bg-indigo-900/30 border-indigo-400">
          ...
          <p className="text-indigo-200 whitespace-pre-wrap">
            {snapshotDescription}
          </p>
        </Card>
      ...
      <div className="pdf-page-break-after">
        <Card>
          <SectionTitle>
            Core Vibrations Based on Vedic Kundli Grid
          </SectionTitle>
              ...
              return (
                <div key={num} className="p-3 bg-gray-900/50 rounded-md">
                  <h4 className="flex items-center">
                    ...
                    {num === report.basicNumber && !isDestiny && (
                    ...
                    {isDestiny && (
                      <span className="ml-2 text-xs font-normal bg-yellow-500/20 text-yellow-300 px-2 py-1 rounded-full">
                        Destiny Number
                      </span>
                    )}
                    {num === report.basicNumber && !isDestiny && (
                      <span className="ml-2 text-xs font-normal bg-rose-500/20 text-rose-300 px-2 py-1 rounded-full">
                        Basic Number
                      </span>
                    )}
                  </h4>
                  ...
                  {report.baseKundliGrid[num] > 1 && (
                    <p className="text-xs text-cyan-300 italic mt-1">
                      This number appears multiple times in the grid, amplifying
                      its influence.
                    </p>
                  )}
                </div>
                ...
                <div key={idx} className="bg-gray-900/30 p-3 rounded-md">
                  ...
                  {yoga.traits && (
                    <ul className="list-disc list-inside text-xs mt-1 text-white/70">
                      {yoga.traits.map((trait, tIdx) => (
                        <li key={tIdx}>{getText(trait, language)}</li>
                      ))}
                    </ul>
                  )}
                </div>
