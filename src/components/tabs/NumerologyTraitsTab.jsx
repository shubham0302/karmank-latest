...
const NumerologyTraitsTab = ({ report, gender, language = 'en' }) => {
    ...
    const TraitItem = ({ label, value, fullWidth = false, highlight = false }) => (
        <div className={`relative overflow-hidden rounded-lg border border-white/10 ${
            ...
            <div className="flex items-start gap-3">
                <span className="text-xl flex-shrink-0 mt-0.5 print:text-base">{traitIcons[label] || '📋'}</span>
                <div className="flex-1 min-w-0">
                    <h4 className={`font-bold mb-2 text-base print:text-sm ${
                        highlight ? 'text-purple-300' : 'text-yellow-400'
                    }`}>
                        {label}
                    </h4>
                    <p className="text-white/90 leading-relaxed text-sm print:text-xs">{value}</p>
                </div>
            </div>
    ...
    const ProfessionColumn = ({ title, items, type }) => {
        ...
        return (
            <div className={`relative p-6 rounded-xl border ${styles.borderColor} ${styles.bg} h-full transition-all hover:shadow-lg hover:shadow-purple-500/10 print:p-4`}>
                {/* Decorative top accent */}
                <div className={`absolute top-0 left-0 right-0 h-1 ${styles.bg} opacity-70`} />

                <div className="flex items-center justify-center gap-2 mb-4 print:mb-3">
                    <span className={`text-lg ${styles.text} print:text-base`}>{styles.icon}</span>
                    <h4 className={`font-bold ${styles.text} text-lg print:text-sm`}>{title}</h4>
                </div>

                <ul className="space-y-3 print:space-y-2">
                    {items.map((item, index) => (
                        <li key={getText(item, language)} className="flex items-start gap-2 text-left group">
                            <span className={`${styles.text} text-xs mt-0.5 opacity-60 group-hover:opacity-100 transition-opacity print:text-[10px]`}>
                                {styles.icon}
                            </span>
        ...
        <div className="space-y-0">
            ...
            <Card className="pdf-page-break-after mb-12" style={{ pageBreakAfter: 'always', pageBreakInside: 'avoid' }}>
                <div className="text-center mb-10 print:mb-6">
                    ...
                    <p className="text-white/60 text-sm mt-4 print:text-xs print:mt-2">Complete Numerological Profile</p>
                </div>
                ...
                <div className="flex items-center gap-4 my-8">
                    ...
                    <span className="text-purple-400 text-sm font-medium">Core Attributes</span>
                    <div className="flex-1 h-px bg-gradient-to-r from-transparent via-purple-500/50 to-transparent" />
                </div>
            ...
            <Card className="pdf-page-break-after mt-12 mb-12" style={{ pageBreakAfter: 'always', pageBreakBefore: 'always', pageBreakInside: 'avoid' }}>
                ...
                <div className="text-center mb-10 print:mb-6">
                    ...
                    <p className="text-white/60 text-sm mt-4 print:text-xs print:mt-2">Growth Areas & Relationships</p>
                </div>

                {/* Divider */}
                <div className="flex items-center gap-4 mb-8">
                    <div className="flex-1 h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />
                    <span className="text-amber-400 text-base font-semibold">Areas for Growth</span>
                    <div className="flex-1 h-px bg-gradient-to-r from-transparent via-amber-500/50 to-transparent" />
                </div>
                ...
                <div className="flex items-center gap-4 my-8">
                    ...
                    <span className="text-pink-400 text-base font-semibold">Relationship Profile</span>
                    <div className="flex-1 h-px bg-gradient-to-r from-transparent via-pink-500/50 to-transparent" />
                </div>
                ...
                <>
                    <Card className="pdf-page-break-after mt-12 mb-12" style={{ pageBreakAfter: 'always', pageBreakBefore: 'always', pageBreakInside: 'avoid' }}>
                        <div className="text-center mb-10 print:mb-6">
                            ...
                            <p className="text-white/60 text-sm mt-4 print:text-xs print:mt-2">Professional Path Guidance</p>
                        </div>
                        ...
                        <div className="space-y-10">
                            ...
                            <div className="flex items-center gap-4 my-8">
                                ...
                                <span className="text-blue-400 text-base font-semibold">Alternative Options</span>
                                <div className="flex-1 h-px bg-gradient-to-r from-transparent via-blue-500/40 to-transparent" />
                            </div>
                    ...
                    <Card className="pdf-page-break-after mt-12 mb-12" style={{ pageBreakAfter: 'always', pageBreakBefore: 'always', pageBreakInside: 'avoid' }}>
                        <div className="text-center mb-10 print:mb-6">
                            ...
                            <p className="text-white/60 text-sm mt-4 print:text-xs print:mt-2">Numerological Support for Career Success</p>
                        </div>
                        ...
                        <div className="relative overflow-hidden rounded-xl border border-indigo-500/30 bg-gradient-to-br from-indigo-900/40 via-purple-900/40 to-blue-900/40 p-8 print:p-6">
                            ...
                            <div className="relative z-10">
                                <div className="flex items-center justify-center gap-3 mb-8 print:mb-6">
                                    ...
                                    <h4 className="font-bold text-indigo-300 text-2xl print:text-xl">Required Number Support</h4>
                                </div>
                                ...
                                <div className="flex justify-center flex-wrap gap-5 print:gap-3 mb-10 print:mb-8">
                                    {supportAnalysis.map(item => (
                                        ...
                                        >
                                            <span className="text-xl print:text-base">{item.isPresent ? '✓' : '!'}</span>
                                            <span className="text-base font-bold print:text-sm">
                                                Number {item.number}
                                            </span>
                                            <span className={`text-sm print:text-xs ${item.isPresent ? 'text-emerald-200' : 'text-rose-200'}`}>
                                ...
                                <div className="text-center mt-10 print:mt-8">
                                    <p className="text-white/70 text-base italic print:text-sm mb-4 print:mb-3">
                                        These numbers provide supportive energy for your career success
                                    </p>
                                    <p className="text-white/50 text-sm print:text-xs">
                                        {supportAnalysis.filter(item => item.isPresent).length} of {supportAnalysis.length} support numbers are present in your chart
                                    </p>
                                </div>
                            </div>
                        </div>
