...
const ForecastTab = ({ report, dashaReport, gender, language = 'en' }) => {
    ...
    const dashaReportMatches = useMemo(() => {
        if (!report || !dashaReport || !dashaReport.yearlyDashaTimeline || dashaReport.yearlyDashaTimeline.length === 0) {
        ...
        console.log('[ForecastTab] Validating dashaReport match:', { reportBirthYear, dashaFirstYear, matches });
    ...
    if (!dashaReportMatches) {
        return (
            <div className="space-y-0">
                <Card className="mb-6">
                    <p className="text-yellow-200/80 text-center py-8">Loading forecast data...</p>
                </Card>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-0">
            ...
            <div className="print:hidden">
                ...
                <div>
                    {activeSubTab === 'Profession' && <ProfessionForecastTab key={`profession-${forecastYear}`} report={report} dashaReport={dashaReport} gender={gender} targetDate={targetDate} language={language} />}
                    {activeSubTab === 'Travel' && <TravelForecastTab key={`travel-${forecastYear}`} report={report} dashaReport={dashaReport} targetDate={targetDate} language={language} />}
                    {activeSubTab === 'Property' && <PropertyForecastTab key={`property-${forecastYear}`} report={report} dashaReport={dashaReport} targetDate={targetDate} language={language} />}
                    {activeSubTab === 'Marriage' && <MarriageForecastTab key={`marriage-${forecastYear}`} report={report} dashaReport={dashaReport} targetDate={targetDate} language={language} />}
                    {activeSubTab === 'Child Birth' && <ChildBirthForecastTab key={`childbirth-${forecastYear}`} report={report} dashaReport={dashaReport} gender={gender} targetDate={targetDate} language={language} />}
                </div>
            </div>
