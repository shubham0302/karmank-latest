...
const AdvancedDashaTab = ({ dashaReport, baseKundliGrid, basicNumber, destinyNumber, foundationalYogas, language = 'en' }) => {
    ...
    }, [baseKundliGrid, activeDashaNumbersForKundli, foundationalYogas, destinyNumber, targetDate, currentMaha, currentYearly, currentMonthly, currentDaily, language, activeSubTab]);
    ...
    const yearlyDashaForCurrentMaha = useMemo(() => {
        ...
        return dashaReport.yearlyDashaTimeline.filter(yearly =>
            yearly.startDate >= currentMaha.startDate && yearly.endDate <= currentMaha.endDate
        );
    ...
    const yearlyDashaAnalysis = useMemo(() => {
        ...
        return { dynamicYogas, traitAnalysis, combinedNote, dynamicMultiplicityAnalysis, tempGrid };
    }, [currentMaha, currentYearly, baseKundliGrid, foundationalYogas, destinyNumber, language]);
    ...
    const renderDynamicTimeline = () => {
        if (activeSubTab === 'maha') {
                    ...
                    <div className="relative p-6">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-xl font-bold text-purple-300">Maha Dasha Timeline</h4>
                            </div>
                            <span className="text-xs text-purple-400/70 uppercase tracking-wider">Long-term Cycles</span>
                        </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-purple-500/30">
                                        <th className="p-3 text-purple-300 text-sm font-semibold">Dasha Number</th>
                                        <th className="p-3 text-purple-300 text-sm font-semibold">Planet</th>
                                        <th className="p-3 text-purple-300 text-sm font-semibold">Start Date</th>
                                        <th className="p-3 text-purple-300 text-sm font-semibold">End Date</th>
                                        <th className="p-3 text-purple-300 text-sm font-semibold">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {dashaReport.mahaDashaTimeline.map((maha, i) => {
                                        const isActive = currentMaha && maha.startDate.getTime() === currentMaha.startDate.getTime();
                                        const isPast = maha.endDate < targetDate;
                                        const isFuture = maha.startDate > targetDate;

                                        return (
                                            <tr key={i} className={`border-b border-purple-500/10 transition-all hover:bg-purple-500/10 ${
                                                isActive ? 'bg-purple-500/20 shadow-lg shadow-purple-500/20' : ''
                                            }`}>
                                                <td className="p-3">
                                                    <span className={`text-2xl font-bold ${isActive ? 'text-purple-300' : 'text-white/70'}`}>
                                                        {maha.dashaNumber}
                                                    </span>
                                                </td>
                                                <td className="p-3 text-white/80 text-sm">
                                                    {getText(DATA.numberDetails[maha.dashaNumber].name, language)}
                                                </td>
                                                <td className="p-3 text-white/70 text-sm">{formatDate(maha.startDate)}</td>
                                                <td className="p-3 text-white/70 text-sm">{formatDate(maha.endDate)}</td>
                                                <td className="p-3">
                                                    {isActive && <span className="px-3 py-1 rounded-full bg-purple-500/30 text-purple-300 text-xs font-semibold flex items-center gap-2 w-fit">
                                                        <div className="w-2 h-2 rounded-full bg-purple-400 animate-pulse" />
                                                        ACTIVE
                                                    </span>}
                                                    {isPast && <span className="text-white/40 text-xs">Completed</span>}
                                                    {isFuture && <span className="text-cyan-400/70 text-xs">Upcoming</span>}
                                                </td>
                                            </tr>
                                        );
                                    })}
                    ...
                    <div className="relative p-6">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-xl font-bold text-indigo-300">Annual Dasha Timeline</h4>
                            </div>
                            <span className="text-xs text-indigo-400/70 uppercase tracking-wider">Yearly Cycles</span>
                        </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-indigo-500/30">
                                        <th className="p-3 text-indigo-300 text-sm font-semibold">Year</th>
                                        <th className="p-3 text-indigo-300 text-sm font-semibold">Dasha Number</th>
                                        <th className="p-3 text-indigo-300 text-sm font-semibold">Planet</th>
                                        <th className="p-3 text-indigo-300 text-sm font-semibold">Start Date</th>
                                        <th className="p-3 text-indigo-300 text-sm font-semibold">End Date</th>
                                        <th className="p-3 text-indigo-300 text-sm font-semibold">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {yearlyDashaForCurrentMaha.map((yearly, i) => {
                                        const isActive = currentYearly && yearly.startDate.getTime() === currentYearly.startDate.getTime();
                                        const isPast = yearly.endDate < targetDate;
                                        const isFuture = yearly.startDate > targetDate;

                                        return (
                                            <tr key={i} className={`border-b border-indigo-500/10 transition-all hover:bg-indigo-500/10 ${
                                                isActive ? 'bg-indigo-500/20 shadow-lg shadow-indigo-500/20' : ''
                                            }`}>
                                                <td className="p-3 text-white/80 font-semibold">{yearly.year}</td>
                                                <td className="p-3">
                                                    <span className={`text-2xl font-bold ${isActive ? 'text-indigo-300' : 'text-white/70'}`}>
                                                        {yearly.dashaNumber}
                                                    </span>
                                                </td>
                                                <td className="p-3 text-white/80 text-sm">
                                                    {getText(DATA.numberDetails[yearly.dashaNumber].name, language)}
                                                </td>
                                                <td className="p-3 text-white/70 text-sm">{formatDate(yearly.startDate)}</td>
                                                <td className="p-3 text-white/70 text-sm">{formatDate(yearly.endDate)}</td>
                                                <td className="p-3">
                                                    {isActive && <span className="px-3 py-1 rounded-full bg-indigo-500/30 text-indigo-300 text-xs font-semibold flex items-center gap-2 w-fit">
                                                        <div className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />
                                                        ACTIVE
                                                    </span>}
                                                    {isPast && <span className="text-white/40 text-xs">Completed</span>}
            ...
            const currentIndex = monthlyDashas.findIndex(m => currentMonthly && m.startDate.getTime() === currentMonthly.startDate.getTime());
                    ...
                    <div className="relative p-6">
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="text-xl font-bold text-cyan-300">Monthly Dasha Timeline</h4>
                            </div>
                            <span className="text-xs text-cyan-400/70 uppercase tracking-wider">Pratyantara Periods</span>
                        </div>
                            ...
                            <table className="w-full text-left">
                                <thead>
                                    <tr className="border-b border-cyan-500/30">
                                        <th className="p-3 text-cyan-300 text-sm font-semibold">Dasha Number</th>
                                        <th className="p-3 text-cyan-300 text-sm font-semibold">Planet</th>
                                        <th className="p-3 text-cyan-300 text-sm font-semibold">Start Date</th>
                                        <th className="p-3 text-cyan-300 text-sm font-semibold">End Date</th>
                                        <th className="p-3 text-cyan-300 text-sm font-semibold">Duration</th>
                                        <th className="p-3 text-cyan-300 text-sm font-semibold">Status</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    {displayMonthly.map((monthly, i) => {
                                        const isActive = currentMonthly && monthly.startDate.getTime() === currentMonthly.startDate.getTime();
                                        const isPast = monthly.endDate < targetDate;
                                        const isFuture = monthly.startDate > targetDate;
                                        const duration = Math.ceil((monthly.endDate - monthly.startDate) / (1000 * 60 * 60 * 24));

                                        return (
                                            <tr key={i} className={`border-b border-cyan-500/10 transition-all hover:bg-cyan-500/10 ${
                                                isActive ? 'bg-cyan-500/20 shadow-lg shadow-cyan-500/20' : ''
                                            }`}>
                                                <td className="p-3">
                                                    <span className={`text-2xl font-bold ${isActive ? 'text-cyan-300' : 'text-white/70'}`}>
                                                        {monthly.dashaNumber}
                                                    </span>
                                                </td>
                                                <td className="p-3 text-white/80 text-sm">
                                                    {getText(DATA.numberDetails[monthly.dashaNumber].name, language)}
                                                </td>
                                                <td className="p-3 text-white/70 text-sm">{formatDate(monthly.startDate)}</td>
                                                <td className="p-3 text-white/70 text-sm">{formatDate(monthly.endDate)}</td>
                                                <td className="p-3 text-white/60 text-xs">{duration} days</td>
                                                <td className="p-3">
                                                    {isActive && <span className="px-3 py-1 rounded-full bg-cyan-500/30 text-cyan-300 text-xs font-semibold flex items-center gap-2 w-fit">
                                                        <div className="w-2 h-2 rounded-full bg-cyan-400 animate-pulse" />
                                                        ACTIVE
                                                    </span>}
                                                    {isPast && <span className="text-white/40 text-xs">Completed</span>}
                ...
                <div className="relative overflow-hidden rounded-xl border border-yellow-500/30 bg-gradient-to-br from-yellow-900/20 via-amber-900/20 to-orange-900/20 backdrop-blur-sm">
                    ...
                    <div className="relative p-8 text-center">
                        <div className="flex items-center justify-center gap-3 mb-6">
                            ...
                            <h4 className="text-2xl font-bold text-yellow-300">Daily Dasha</h4>
                            <div className="w-3 h-3 rounded-full bg-yellow-400 animate-pulse" />
                        </div>

                        {currentDaily ? (
                            <div className="max-w-md mx-auto">
                                <p className="text-white/70 mb-4 text-sm">Dasha for {formatDate(targetDate)}</p>
                                <div className="relative inline-block">
                                    <div className="absolute inset-0 bg-yellow-400/20 blur-2xl" />
                                    <p className="relative text-8xl font-bold text-yellow-400 my-6">{currentDaily.dashaNumber}</p>
                                </div>
                                <p className="text-yellow-300 font-semibold text-xl mt-4">
                                    {getText(DATA.numberDetails[currentDaily.dashaNumber].name, language)}
                                </p>
                                <div className="mt-6 p-4 bg-yellow-500/10 rounded-lg border border-yellow-500/20">
                                    <p className="text-white/80 text-sm leading-relaxed">
                                        {getText(DATA.numberDetails[currentDaily.dashaNumber].description, language)}
                                    </p>
                                </div>
                            </div>
                        ) : (
                            <p className="text-white/70">No daily dasha data available for this date</p>
                        )}
                    </div>
                </div>
            );
    ...
    return (
        <div className="space-y-0">
            ...
            <Card className="pdf-page-break-after mb-12" style={{ pageBreakAfter: 'always', pageBreakInside: 'avoid' }}>
                <div className="text-center mb-10 print:mb-6">
                    ...
                    <p className="text-white/60 text-sm mt-4 print:text-xs print:mt-2">
                        Influences active for year {currentYearly?.year || targetDate.getFullYear()}
                    </p>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6 mb-8 print:gap-4 print:mb-6 print:mb-0">
                    ...
                    <div className="bg-gradient-to-br from-purple-900/40 to-indigo-900/40 p-6 rounded-xl border border-purple-500/30 print:p-4">
                        <div className="text-center">
                            <h3 className="text-xl font-bold text-purple-300 mb-3 print:text-lg print:mb-2">Current Maha Dasha</h3>
                            {currentMaha ? (
                                <>
                                    <p className="text-5xl font-bold text-purple-400 my-3 print:text-3xl print:my-2">{currentMaha.dashaNumber}</p>
                                    <p className="text-white/70 text-sm print:text-xs">
                                        {formatDate(currentMaha.startDate)} to {formatDate(currentMaha.endDate)}
                                    </p>
                                    <p className="text-purple-300 font-semibold mt-2 text-base print:text-sm print:mt-1">
                                        {getText(DATA.numberDetails[currentMaha.dashaNumber].name, language)}
                                    </p>
                                </>
                    ...
                    <div className="bg-gradient-to-br from-indigo-900/40 to-blue-900/40 p-6 rounded-xl border border-indigo-500/30 print:p-4">
                        <div className="text-center">
                            <h3 className="text-xl font-bold text-indigo-300 mb-3 print:text-lg print:mb-2">Current Annual Dasha</h3>
                            {currentYearly ? (
                                <>
                                    <p className="text-5xl font-bold text-indigo-400 my-3 print:text-3xl print:my-2">{currentYearly.dashaNumber}</p>
                                    <p className="text-white/70 text-sm print:text-xs">
                                        {formatDate(currentYearly.startDate)} to {formatDate(currentYearly.endDate)}
                                    </p>
                                    <p className="text-indigo-300 font-semibold mt-2 text-base print:text-sm print:mt-1">
                                        {getText(DATA.numberDetails[currentYearly.dashaNumber].name, language)}
                                    </p>
                                </>
            ...
            {Object.keys(dynamicAnalysis.traitAnalysis).length > 0 && (
                <Card className="pdf-page-break-after mt-12 mb-12" style={{ pageBreakAfter: 'always', pageBreakBefore: 'always', pageBreakInside: 'avoid' }}>
                    <div className="text-center mb-10 print:mb-6">
                        <div className="inline-block relative">
                    ...
                    <div className="space-y-4">
                        {dynamicAnalysis.combinedNote && (
                            <div className="p-4 bg-fuchsia-500/20 border-l-4 border-fuchsia-400 rounded-r-lg print:bg-transparent print:border-l-2">
                                <h4 className="font-bold text-fuchsia-300 text-base print:text-sm">Combined Insight</h4>
                                <p className="text-sm text-white/90 mt-1 print:text-xs">{dynamicAnalysis.combinedNote}</p>
                            </div>
                        )}
                        {Object.entries(dynamicAnalysis.traitAnalysis).map(([number, data]) => (
                            <div key={number} className="p-4 bg-gray-900/50 rounded-md border border-white/10 print:bg-transparent print:p-3">
                                ...
                                <p className="text-xs text-yellow-200/80 italic mb-2 print:text-xs">
                                    ...
                                    {data.sources.length > 1 && <span className="font-bold text-red-400"> (Amplified)</span>}
                                </p>
                                <p className="text-sm text-white/90 print:text-xs">{getText(data.details.description, language)}</p>
                            </div>
                        ))}
                    </div>
                </Card>
            ...
            {dynamicAnalysis.dynamicYogas.length > 0 && (
                <Card className="pdf-page-break-after mt-12 mb-12" style={{ pageBreakAfter: 'always', pageBreakBefore: 'always', pageBreakInside: 'avoid' }}>
                    <div className="text-center mb-10 print:mb-6">
                        ...
                        <p className="text-white/60 text-sm mt-4 print:text-xs print:mt-2">
                            Yogas formed by current {activeSubTab === 'maha' ? 'Maha Dasha' : activeSubTab === 'yearly' ? 'Maha + Annual Dasha' : activeSubTab === 'monthly' ? 'Maha + Annual + Monthly Dasha' : 'All Active Dasha'} combination
                        </p>
                    </div>

                    <div className="space-y-4">
                        {dynamicAnalysis.dynamicYogas.map(yoga => (
                            <div key={getText(yoga.name, language)} className="bg-gradient-to-br from-green-900/20 to-emerald-900/20 p-6 rounded-xl border border-green-500/30 print:p-4 print:bg-transparent">
                                <h3 className="text-xl font-bold text-yellow-400 mb-3 print:text-lg">{getText(yoga.name, language)}</h3>
                                <p className="text-base text-white/90 leading-relaxed print:text-sm">{getText(yoga.description, language)}</p>
                                {yoga.traits && yoga.traits.length > 0 && (
                                    <div className="mt-4">
                                        <h4 className="text-sm font-semibold text-green-300 mb-2 print:text-xs">Key Traits:</h4>
            ...
            <Card className="pdf-page-break-after mt-12 mb-12" style={{ pageBreakAfter: 'always', pageBreakBefore: 'always', pageBreakInside: 'avoid' }}>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-indigo-500/30">
                                <th className="p-3 text-indigo-300 text-sm font-semibold print:p-2 print:text-xs">Year</th>
                                <th className="p-3 text-indigo-300 text-sm font-semibold print:p-2 print:text-xs">Dasha Number</th>
                                <th className="p-3 text-indigo-300 text-sm font-semibold print:p-2 print:text-xs">Planet</th>
                                <th className="p-3 text-indigo-300 text-sm font-semibold print:p-2 print:text-xs">Start Date</th>
                                <th className="p-3 text-indigo-300 text-sm font-semibold print:p-2 print:text-xs">End Date</th>
                                <th className="p-3 text-indigo-300 text-sm font-semibold print:p-2 print:text-xs print:hidden">Status</th>
                            </tr>
                        </thead>
                        <tbody>
                            {yearlyDashaForCurrentMaha.map((yearly, i) => {
                                ...
                                return (
                                    <tr key={i} className={`border-b border-indigo-500/10 ${
                                        ...
                                        <td className="p-3 text-white/80 font-semibold print:p-2 print:text-xs">{yearly.year}</td>
                                        <td className="p-3 print:p-2">
                                            <span className={`text-2xl font-bold print:text-lg ${isActive ? 'text-indigo-300' : 'text-white/70'}`}>
                                                {yearly.dashaNumber}
                                            </span>
                                        </td>
                                        ...
                                        <td className="p-3 text-white/70 text-sm print:p-2 print:text-xs">{formatDate(yearly.startDate)}</td>
                                        <td className="p-3 text-white/70 text-sm print:p-2 print:text-xs">{formatDate(yearly.endDate)}</td>
                                        <td className="p-3 print:hidden">
                                            {isActive && <span className="px-3 py-1 rounded-full bg-indigo-500/30 text-indigo-300 text-xs font-semibold">ACTIVE</span>}
                                            {isPast && <span className="text-white/40 text-xs">Completed</span>}
                                            {isFuture && <span className="text-cyan-400/70 text-xs">Upcoming</span>}
                                        </td>
                                    </tr>
            ...
            <div className="print:hidden">
                ...
                <div className="mb-8">
                    <div className="bg-gradient-to-br from-purple-900/40 via-indigo-900/30 to-blue-900/40 border border-purple-500/30 rounded-2xl p-8 shadow-2xl">
                        ...
                        <div className="mt-8 p-4 bg-gray-900/50 rounded-xl border border-gray-700/50">
                            <h4 className="text-sm font-semibold text-center text-purple-300 mb-4 uppercase tracking-wider">Active Dasha Legend</h4>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded" style={{ backgroundColor: DATA.colorMap.maha }}></div>
                                    <span className="text-white/80">Maha Dasha ({currentMaha ? currentMaha.dashaNumber : 'N/A'})</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded" style={{ backgroundColor: DATA.colorMap.yearly }}></div>
                                    <span className="text-white/80">Yearly Dasha ({currentYearly ? currentYearly.dashaNumber : 'N/A'})</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded" style={{ backgroundColor: DATA.colorMap.monthly }}></div>
                                    <span className="text-white/80">Monthly Dasha ({currentMonthly ? currentMonthly.dashaNumber : 'N/A'})</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded" style={{ backgroundColor: DATA.colorMap.daily }}></div>
                                    <span className="text-white/80">Daily Dasha ({currentDaily ? currentDaily.dashaNumber : 'N/A'})</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded" style={{ backgroundColor: DATA.colorMap.destiny }}></div>
                                    <span className="text-white/80">Destiny ({destinyNumber})</span>
                                </div>
                                <div className="flex items-center gap-2">
                                    <div className="w-4 h-4 rounded" style={{ backgroundColor: DATA.colorMap.basic }}></div>
                        ...
                        <div className="mt-6 p-4 bg-gradient-to-r from-cyan-900/30 to-blue-900/30 border border-cyan-500/30 rounded-xl">
                            <div className="text-center">
                                <p className="text-cyan-300 font-semibold mb-2">Currently Active Influences</p>
                                <div className="flex flex-wrap justify-center gap-4 text-sm">
                                    {currentMaha && (
                                        <div className="bg-gray-900/50 px-3 py-1.5 rounded-lg">
                                            <span className="text-white/60">Maha:</span> <span className="text-white font-bold ml-1">{currentMaha.dashaNumber} ({getText(DATA.numberDetails[currentMaha.dashaNumber]?.name, language)})</span>
                                        </div>
                                    )}
                                    {currentYearly && (
                                        <div className="bg-gray-900/50 px-3 py-1.5 rounded-lg">
                                            <span className="text-white/60">Yearly:</span> <span className="text-white font-bold ml-1">{currentYearly.dashaNumber} ({getText(DATA.numberDetails[currentYearly.dashaNumber]?.name, language)})</span>
                                        </div>
                                    )}
                                    {currentMonthly && (
                                        <div className="bg-gray-900/50 px-3 py-1.5 rounded-lg">
                                            <span className="text-white/60">Monthly:</span> <span className="text-white font-bold ml-1">{currentMonthly.dashaNumber} ({getText(DATA.numberDetails[currentMonthly.dashaNumber]?.name, language)})</span>
                                        </div>
                                    )}
                                    {currentDaily && (
                                        <div className="bg-gray-900/50 px-3 py-1.5 rounded-lg">
                                            <span className="text-white/60">Daily:</span> <span className="text-white font-bold ml-1">{currentDaily.dashaNumber} ({getText(DATA.numberDetails[currentDaily.dashaNumber]?.name, language)})</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="mb-6">
                    <div className="flex flex-wrap items-center gap-4 mb-6">
                        <div>
                            <label htmlFor="dasha-date" className="block text-sm font-medium text-yellow-500 mb-1">
