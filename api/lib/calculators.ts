 ...
 * @returns The numerology report or null if DOB is invalid.
 */
export const calculateNumerology = (dob: string): any => {
  if (!dob) return null;
  ...
  const destinyNumber = reduceToSingleDigit(
    dobString.split('').reduce((sum, digit) => sum + parseInt(digit, 10), 0)
  );
  ...
  Object.values(DATA.yogaDetails).forEach((yoga: any) => {
    let isPresent = false;
    if (yoga.activation_rules) {
      ...
      isPresent = yoga.numbers.every((num: number) => digitCounts[num] > 0);
      if (isPresent && yoga.empty && yoga.empty.length > 0) {
        if (yoga.empty.some((num: number) => digitCounts[num] > 0)) {
          isPresent = false;
        }
      }
    }
...
export const dashaCalculator = {
  ...
  calculateYearlyDasha(dob: Date, basicNum: number): any[] {
    const timeline: any[] = [];
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const startDate = new Date(birthdayOfTargetYear);
  ...
  calculateMonthlyDasha(yearlyDashaTimeline: any[]): any[] {
    const timeline: any[] = [];
    ...
    yearlyDashaTimeline.forEach((yearlyDasha) => {
      ...
      while (currentStartDate <= endOfYearPeriod) {
        ...
        const duration = (this.pratyantarDurationMap as any)[currentDashaNum];
  ...
  calculateDailyDasha(monthlyDashaTimeline: any[]): any[] {
    const timeline: any[] = [];
    monthlyDashaTimeline.forEach(({ dashaNumber: pratyantarNum, startDate, endDate }: any) => {
